  ...
  return (
    ...
    >
      <View className="flex-row justify-between items-center mb-3">
        <View className="flex-row items-center flex-1">
          <View className="w-10 h-10 bg-primary rounded-full mr-3 justify-center items-center">
            {authorImage ? (
              ...
              <Text className="text-white font-bold">{authorName.charAt(0)}</Text>
            )}
          </View>
        ...
        <View className={`px-3 py-1 rounded-full ${type === 'workout' ? 'bg-primary bg-opacity-20' : 'bg-yellow-500 bg-opacity-20'}`}>
          <Text className={`text-xs font-semibold ${type === 'workout' ? 'text-primary' : 'text-yellow-400'}`}>
      ...
      {caloriesBurned && (
        <View className="bg-background rounded-lg p-3 mb-3 flex-row items-center">
          ...
          <Text className="text-white font-semibold ml-2">+{caloriesBurned} kcal</Text>
        </View>
      )}
      ...
      <View className="flex-row justify-between items-center pt-3 border-t border-gray-800">
        ...
        >
          ...
          />
          <Text className={`ml-2 font-semibold ${isLiked ? 'text-primary' : 'text-gray-400'}`}>
            {likes}
          </Text>
        </TouchableOpacity>
        ...
        >
          ...
          />
          <Text className={`ml-2 font-semibold ${isSaved ? 'text-primary' : 'text-gray-400'}`}>
            Opslaan
          </Text>
        </TouchableOpacity>
