  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        setStats({ workouts: count, minutes: estMinutes, friends: friendsCount });
        setLoading(false);
      ...
      <ScrollView className="px-4">
        ...
        <View className="flex-row justify-between bg-surface p-6 rounded-2xl border border-gray-800 mb-6">
          <View className="items-center flex-1">
            {loading ? <ActivityIndicator size="small" color="white"/> : <Text className="text-white text-xl font-bold">{stats.workouts}</Text>}
            <Text className="text-gray-400 text-xs mt-1">Workouts</Text>
          </View>
          <View className="w-px bg-gray-700" />
          <View className="items-center flex-1">
            {loading ? <ActivityIndicator size="small" color="white"/> : <Text className="text-white text-xl font-bold">{stats.minutes}</Text>}
            <Text className="text-gray-400 text-xs mt-1">Minuten</Text>
          </View>
          <View className="w-px bg-gray-700" />
          <View className="items-center flex-1">
            {loading ? <ActivityIndicator size="small" color="white"/> : <Text className="text-white text-xl font-bold">{stats.friends}</Text>}
